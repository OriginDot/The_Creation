...
void decide_direct(struct predator* self, int (*map)[MAP_HEIGHT]) {
	int i, j;
	int target_x = 0x7FFFFFFF; /*							*/
	int target_y = 0x7FFFFFFF; /*	max value of integer	*/
...
void decide_gene(struct predator* self) {
	...
	if(heredity_seed < PREDATOR_ODD_HEREDITY_PROBABILITY) { /* 1 */
		gene_seed = 
			arithmetic_roulette_select(PREDATOR_ODD_HEREDITY_SEED);
		goto DECIDE_GENE_END;
	}
	...
	memcpy(	self->own_gene, 
		get_index_node(self->parent_genes, gene_seed)->data,
		sizeof(struct predator_gene)				);
