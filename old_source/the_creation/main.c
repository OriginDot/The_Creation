...
void init(int (*map)[MAP_HEIGHT]) {
	...
	for(i = 0; i < PREDATOR_INIT_CNT; ++i) {
		...
		struct predator* new_predator = 
			(struct predator*)
				malloc(sizeof(struct predator));
		struct node* new_predator_node = 
			(struct node*)
				malloc(sizeof(struct node));
		struct predator_gene* new_gene = 
			(struct predator_gene*)
				malloc(sizeof(struct predator_gene));
		struct node* new_gene_node = 
			(struct node*)
				malloc(sizeof(struct node));
		struct down_sorted_list* new_parent_gene = 
			(struct down_sorted_list*)
				malloc(sizeof(struct down_sorted_list));
		...
		new_predator_node->sort_index = 
				new_predator->own_gene->achievement_cnt;
...
void process(int (*map)[MAP_HEIGHT]) {
	...
	for(i = 0; i < size; ++i) {
		struct predator* tmp;
		tmp = (struct predator*)
			get_index_node(&predator_storage, i)->data;
